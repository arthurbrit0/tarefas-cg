...
Vetor3D Vetor3D::operator-(const Vetor3D& v) const {
     return Vetor3D(x - v.x, y - v.z, z - v.z);                                                         // implementando a subtracao entre dois vetores, que retorna um vetor com a subtracao de cada componente
};
...
Vetor3D Vetor3D::operator-() const {                                                                    // implementando a negacao de um vetor, retornando um novo vetor com as componentes negativas do original  
     return Vetor3D(-x, -y, -z); 
};

float Vetor3D::dot(const Vetor3D& v) const {                                                            // implementando o produto escalar entre dois vetores, retornando um float
     return x * v.x + y * v.y + z * v.z;                                                                // a soma dos produtos das componentes de cada vetor
};

float Vetor3D::norma() const {                                                                          // implementando a funcao para calcular a norma de um vetor, retornando um float
     return std::sqrt(this->dot(*this));                                                                // a raiz quadrada do produto escalar do vetor com ele mesmo
};

Vetor3D Vetor3D::normalizar() const {                                                                   // implementando a funcao para normalizar um vetor, ou seja, deixa-lo unitario
...
Raio::Raio(Vetor3D origemVetor, Vetor3D extremidadeVetor) : origem(origemVetor) {                       // implementando a funcao construtora da classe Raio, definindo o valor da origem como o vetor origemVetor
    Vetor3D dir = extremidadeVetor - origem;                                                            // o vetor direção é a extremidade do vetor - sua origem
    direcao = dir.normalizar();                                                                         // para obter apenas a direção, sem nos importarmos com o comprimento, normalizamos o vetor direcao (tornamos-o unitario)                           
}
...
bool Esfera::intersecao(const Raio &raio, float &t) const {                                             // implementando a funcao para verificar se há intersecao entre a esfera e o raio
    ...
    if (delta > 0) {                                                                                    // se o delta for maior que 0, há intersecao
        ...
        if (t_atual > 0 && t_atual < t) {                                                               // validando se essa raiz realmente esta no intervalo de interesse
            t = t_atual;
            return true;
        }
    } else if (delta == 0) {                                                                            // se o delta for igual a 0, há intersecao tangencial
        float t1 = -b / (2.0f * a);                                                                     // fórmula para calcular a unica raiz
        if (t1 > 0 && t1 < t) {                                                                         // validando se essa raiz realmente esta no intervalo de interesse
            t = t1;
            return true;
        }
    }
